...
A project directory named `java-data-type-max-value` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-data-type-max-value` directory which is created just now.
